...
class App_model extends CI_Model {
	...
	public function saov_preview ()	{
		...
		if (is_array($response)) {
	        ...
	        $pdf->Image($img_file, 0, 0, 216, 279, '', '', '', false, 300, '', false, false, 0, false, false, false);
	        // set logo image
			$upload_dir =  $this->config->item ('sbwk_dir');
			$image_logo = base_url ($upload_dir . $response['file_name']);
			
			//Parameter details(1-> "top space", 2->"bottom space", 3->"logo width",m 4->"logo height")
			$pdf->Image($image_logo, 17, 9, 56, 14, '', '', '', false, 300, '', false, false, 0, false, false, false);
	...
	public function sbhanger_preview ($response=[])	{
