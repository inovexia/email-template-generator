...
class Appgen_actions extends CI_Controller {
	...
	public function sbwk ()	{
		...
		if (is_array($response)) {
			...
			$pdf->Image($image_logo, 162, 22, 36, 12, 'JPG', 'http://www.tcpdf.org', '', true, 150, '', false, false, 1, false, false, false);
	...
	public function sbhanger ()	{
		...
		if (is_array($response)) {
			...
			$upload_dir =  $this->config->item ('sbwk_dir');
			$image_logo = base_url ($upload_dir . $response['file_name']);
			$pdf->Image($image_logo, 5, 5, 34, 12, 'JPG', '', '', false, 150, '', false, false, 1, false, false, false);
